class Instructions:
	...
	def READ_FROM_MEM_TO_A(self, *address):
		address = int(address[0]) if address else eval(input('Enter address to read from (0-255): '))
		return [0, address]
	...
	def WRITE_FROM_A_TO_MEM(self, *address):
		address = int(address[0]) if address else eval(input('Enter address to write to (0-255): '))
		return [2, address]
	...
	def ALU_OPERATION_REG_A(self, *op_type):
		op_type = str(op_type[0]) if op_type else input('Choose operation type (+, -, *, <<, >>, A+1, B+1, A-1, B-1, A==B, A>B, A<B: ')
	...
	def BREQ_ADDR(self, *address):
		address = int(address[0]) if address else eval(input('Enter address to branch to (0-255): '))
		return [150, address]
	...
	while(1):
		...
		if command in possible_commands and no_command <= 255:
			if command not in additional_commands:
				...
				address = eval(input('Enter address to return after interrupt: '))
				...
				instructions_filename = input('Enter instructions filename: ')
